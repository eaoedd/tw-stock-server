...
**Result** — PASS (computation logic via Python port; chart rendering requires manual browser verification)
- `parseAlloc`: 2-ticker, single-ticker, zero-weight, uppercase normalisation
- `btStats`: flat returns (annReturn=0, maxDD=0), constant +0.1%/day (annReturn≈25.2%), declining returns (maxDD>0), sharpe type
- `pearsonCorr`: perfect positive (1.0), perfect negative (-1.0), identical series (1.0)
- `btCompute`: returnDates/portCum length = days−1, stats = N tickers + Portfolio row, N×N correlation matrix, diagonal = 1
- Portfolio save/load: `PUT /portfolios` and `GET /portfolios` round-trip 3 portfolios with correct names and allocs
- History endpoint: `GET /history/0050?period=1mo` returns price records
